<!DOCTYPE html PUBLIC ">
<html>
<head>
<meta http-equiv="; charset=utf-8" />
<title>QuantCasino</title>
<meta name="keywords" content="" />
<meta name="description" content="" />
<link href="templatemo_style.css" rel="stylesheet" type="text/css" />
<link href="css/jquery.ennui.contentslider.css" rel="stylesheet" type="text/css" media="screen,projection" />
<script language="javascript" type="text/javascript">
function clearText(field)
{
    if (field.defaultValue == field.value) field.value = '';
    else if (field.value == '') field.value = field.defaultValue;
}
</script>
</head>  
<body style="background:url(кс.png);"
 background-repeat: no-repeat; background-position: center center;
 background-attachment: fixed;
 -webkit-background-size: cover;
 -moz-background-size: cover;
 -o-background-size: cover;
 background-size: cover;>
</body> 


<div id="site_title"></div>
    <a href=""><img src="основапенипукпук.png" alt="" /><span></span></a>
</div> <!-- end of site_title -->
<div id="templatemo_menu">
            <ul>
                
                
                <li><a href="about.php" class="current">Игры </a></li>
                <li><a href="portfolio.php">Связь</a></li>
                <li><a href="blog.php">Банк</a></li>
                <li class="last"><a href="contact.php">Профиль</a></li>
            </ul>
            
           
                </form>
            </div>
              	
        </div>
  
    </div> <!-- end of header -->
</div>
<div style="display:flex;justify-content:center; width:100%">
<div id="hook" style="background-color: white; height:600px; width:1150px; display:flex;">

    <div class="container">
        <button onclick="window.location.href = 'Новая папка/index.html'"input type="image" src="neon.png" >Click Here</button>
            
        
        
          
          
            
            
            
            
            
    
    
          
        
    
            
        
        
            
            
            
            
            
        
            
        
          
    </div>
    <div id="block">
    
    </div>



     
</div> 

</div><!-- end of wrapper -->
<!-- templatemo 293 liquid -->
<!-- 
Liquid Template 
http://www.templatemo.com/preview/templatemo_293_liquid 
QuantCasino
-->
</body>
</html>

